...
<x-backend.card>
    <x-slot name="header">
       <span style="font-size: 16px"> <strong>@lang('RECOVERY INVOICE : #' . $recovery_invoice->invoice_number)</strong></span>
    </x-slot>
    ...
    <x-slot name="headerActions">
        <div class="row">            
            <div class="col-md-12">
                @if($levels[0]['weight'] == $recovery_invoice->weight && $recovery_invoice->approvalLogs->last()->status != 'rejected' )
                    <p>Pending Your Approval</p>
                @else
                <p>CURRENT LEVEL: {{$recovery_invoice->level}} {{$recovery_invoice->approvalLogs->last()->type}}</p>
                @endif

                <?php $array = str_split(preg_replace('/[^A-Za-z0-9\-]/', '', @$recovery_invoice->edit_fields));?>
                
                
                @if($recovery_invoice->level == @auth()->user()->approvalLevel->weight && $recovery_invoice->approvalLogs->last()->type != 'rejected')
                    <a>
    ...
    <x-slot name="body">        
        <div class="row">
            @if($recovery_invoice->approvalLogs->last()->type == "rejected")
                <div class="col-md-12 alert alert-danger" style="border-radius: 0px;">
                    <p class="text-center" style="font-size: 24px;">CURRENT STATUS: <span style="font-weight: bold; text-decoration: underline; text-align: center; font-size: 24px;">REJECTED by {{$recovery_invoice->approvalLogs->last()->user->name}}</span></p>
                    <p class="text-center">Comments: <strong>{{@$recovery_invoice->recalled->comments}}</strong></p>
                    <?php $code = @$recovery_invoice->approvalLogs->last()->reason->id; ?>
                    @if($code == 1 || $code == 3 || $code == 5 || $code == 10 || $code == 11)
                        <a href="{{route('admin.transactions.edit_invoice', $recovery_invoice->invoice->id)}}"><button class="btn btn-sm btn-info">Edit Invoice</button></a>
                    @endif

                    @if($code == 2 || $code == 4)
                        <a href="{{route('admin.transactions.edit_invoice', $recovery_invoice->invoice->id)}}"><button class="btn btn-sm btn-info">Edit RCN's</button></a>
                    @endif

                    @if($code == 5)
                        <a href="{{route('admin.transactions.edit_invoice', $recovery_invoice->invoice->id)}}"><button class="btn btn-sm btn-info">Resend</button></a>
                    @endif

                    @if($code == 6 || $code == 7 || $code == 8)
                        <a href="{{route('admin.transactions.edit_recovery_invoice', $recovery_invoice->id)}}"><button class="btn btn-sm btn-info">Edit Recovery Invoice</button></a>
                    @endif
                </div>
            @endif
        </div>
                ...
                <div class="col-md-12 col-lg-12 col-sm-12">
                    <table class="table table-bordered table-sm">
                        <tr>
                            ...
                            <th>INV. AMOUNT</th>
                            <td>{{$invoice->currency->symbol}} {{ number_format($invoice->invoice_amount) }}</td>
                        </tr>
                        <tr>
                            ...
                            <td>{{$invoice->currency->symbol}} {{ $invoice->credit_note_amount ? $invoice->credit_note_amount : 'NA' }}</td>
                        </tr>
                        <tr>
                            ...
                            <td>{{$recovery_invoice->currency->symbol}} {{ number_format($recovery_invoice->invoice_amount) }}</td>
                        </tr>
                    </table>
                </div>
            </div>
