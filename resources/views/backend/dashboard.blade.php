            ...
            <table class="table table-sm table-bordered" style="width: 80%">
                ...
                <tfoot>
                    ...
                    <tr style="background-color: red; border: 2px solid #000" class="rowh">
                        <td colspan="4">
                            <p class="text-sm ma-2">RCN booked pending approval process: <br /> USD:{{number_format(@$usd_booked_rcn[0]['amount'] - @$total_usd_approval, 2)}} | KES: {{number_format(@$ksh_booked_rcn[0]['amount'] - @$total_ksh_approval, 2)}}</p>
                        </td>
                    </tr>
                </tfoot>
            </table> -->
                ...
                <table class="table table-striped table-sm table-bordered">
                    <thead>
                        <tr class="text-center rowb rowh">
                            <th colspan="3">TOP TRANSPORTERS</th>
                        </tr>
                        <tr>
                            <th>Name</th>
            ...
            <div class="col-sm-6">
                ...
                <table class="table table-striped table-sm table-bordered" style="width: 80%">
                    ...
                    <tfoot>
                        <tr>
                            <th>TOTAL</th>
                            <td>{{$total_invoices !== 0 ? $total_invoices : 'N/A'}}</td>
                        </tr>
                    </tfoot>
                </table> -->
            <!-- <p>
                <strong>
                    ...
                    @if($total_invoices !== 0)
                    {{ round(($total_approvals / $total_invoices) * 100, 2) }}%
                    @else
                    N/A
                    @endif
                </strong>
                of all invoices being processed.
                <strong>
                    @if($total_invoices !== 0)
                    {{ round(($total_approved / $total_invoices) * 100, 2) }}%
                    @else
                    N/A
                    @endif
                </strong>
