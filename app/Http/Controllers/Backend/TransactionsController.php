...
class TransactionsController extends Controller
{
    ...
    public function attachInvoice()
    {
        ...
        if (request()->has("rcn_no")) {
            ...
            $rs = $this->stripQrCodeValue(request("rcn_no"));

            $rcn = Transaction::where("rcn_no", $rs)->first();            
            if (!$rcn) {
                $status = session("status", true);
                return redirect()->back()->with("flash_danger", "RCN no: " . $rs . " not found. Continue and add transporter name? <a href='".route("admin.transactions.attach-invoice")."?rcns=".$rs.":0"."&status=".$status."'> Yes </a>")->withInput();
            }            
            ...
            if (in_array($rs, $cleaned_rcns)) {
                return redirect()->to(route('admin.transactions.attach-invoice') . '?rcns=' . request('rcns'))->with("flash_danger", "RCN no: " . $rs . " added already");
            }
            // $invoice = request('invoice_id') ?  request('invoice_id') : $rcn->invoice_id;

            if ($rcn->invoice_id) {
                return redirect()->back()->with("flash_danger", "RCN no: " . $rs . " has an invoice attached already. Would you like to proceed? <a href='".route("admin.transactions.attach-invoice")."?rcns=".$rcns."&status=".$status."'> Yes </a>")->withInput();
    ...
    private function stripQrCodeValue($scan)
    {
        $arr = [];
        $value = str_contains($scan, '|');
        if($value)
        {
            $arr = explode('|', $scan);
            return $arr[4];
    ...
    public function updateRecoveryInvoice($invoice_id)
    {
        ...
        if($recover->approvalLogs)
        {
            ...
            $insert = ApprovalLog::create([
                ...
                "comments" => 'Edited:'. request('comments'),
                "weight" => $approver->weight,
                "is_approved" => false,
                "type" => ApprovalLog::$EDITED,
                // "reason_code" => request("reason_code")
    ...
    {
        $permitted = ApprovalLevel::selectRaw("departments, branches")->where("user_id", Auth::id())->first();
        ...
        $invoices = TransactionInvoice::whereHas('rcns', function($query) use ($permitted){
                        $query->whereIn("department_code", $permitted['departments']);
                        // $query->whereIn("branch_code", $permitted["branches"]);
    ...
    {   
        request('download') == 1 ? $limit = 3000 : $limit = request('limit');
        // request('download') ? $limit = 1000 : $limit = 20;
        // return $limit;
    ...
    {   
        ...
        request('download') == 1 ? $limit = 5000 : $limit = 20;
        // if(request('download')){ $limit = }
        // return $limit;
        ...
        if (request('download') == 1) {      
            $view = view('backend.trx.report_export',
                ...
                return Excel::download(new ValidExports($view),  ' Submitted invoices report ' . date('Y-m-h H:i:s') . '.csv');
    ...
    {
        $level = ApprovalLevel::selectRaw("weight, departments, branches")->where("user_id", Auth::id())->first();
        $levels = ApprovalLevel::all();

        request('download') == 1 ? $limit = 2000 : $limit = 20;
        ...
        if(request('status'))    
        {
            $recovery_invoices = RecoveryInvoice::whereHas('invoice.rcns', function($query) use ($level){
                ...
                // $query->whereIn("transactions.branch_code", $level["branches"]);
            })
            // ->where('')
            ->filter(request()->all())->latest()->paginate($limit); 
        }else{
            ...
            ->filter(request()->all())->orderBy("recovery_invoices.level", "desc")->paginate($limit);   
    ...
    {
        request('download') == 1 ? $limit = 1000 : $limit = 20;
