...
class TransactionInvoice extends Model
{
    ...
    public function scopeFilter($query, $request_data)
    {
        ...
        if (array_key_exists("search", $request_data) && $request_data['search']) {
            ...
            $query = $query->where('transaction_invoices.invoice_number', $search)->orWhere('transaction_invoices.file_number', $search)->orWhere('transaction_invoices.tracking_no', $search);
        }
