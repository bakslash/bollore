...
class RecoveryInvoice extends Model
{
    ...
    {   
        ...
        if (array_key_exists("search", $request_data) && $request_data['search']) {
            ...
            $query = $query->whereHas('invoice', function($query) use ($search)
            {
                $query->where('transaction_invoices.invoice_number', $search)->orWhere('transaction_invoices.tracking_no', $search)->orWhere('transaction_invoices.file_number', $search);
